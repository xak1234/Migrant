// --- Firebase Configuration ---
         ...
         let initialBoardLayout = [ // This layout will be reformatted
             ...
             { id: 4, name: "Black Market Sales", type: "set_property", price: 200, rent_base: 150, groupId: "special_set" }, // Rent base is now 150, used per card owned
         ...
         function reformatBoardLayout() { 
             const newBoardLocations = [
                 // Adjusted rent_base for these new special set properties to 150 as well for consistency with the user's request for the original four.
         ...
         async function handleCreateGame() {
             ...
             const initialGameState = {
                 ...
                 lastRentEvent: null, 
                 flashingProperties: [], // Added for property swap flashing
             };
             ...
             // Determine game status based on human player slots
             // Count human players already added (just the host at this point)
             ...
             if (humanPlayersAddedCount >= numHumanPlayers) {
                  // All human slots are filled (e.g., host is the only human and AIs fill the rest)
                  initialGameState.status = "active"; 
                  initialGameState.preGamePhase = true; // Start pre-game rolls
                  initialGameState.lastActionMessage = `${localPlayerName} created the game with ${numAIPlayers} AI(s). Starting pre-game rolls.`;
             } else {
                 // Still waiting for more human players
         ...
         async function handleJoinGame() {
             ...
             try {
                 await runTransaction(db, async (transaction) => {
                     ...
                     // Ensure boardLayout is adopted if joining
                     if (gameData.boardLayout && gameData.boardLayout.length > 0) {
                         boardLayout = gameData.boardLayout;
                         const dcSpace = boardLayout.find(s => s.name === "Detention Center");
                         detentionCenterSpaceId = dcSpace ? dcSpace.id : (boardLayout.find(s => s.type === "detention_visiting")?.id || 8);
                     } else {
                         reformatBoardLayout(); // Fallback if not in gameData
                     ...
                     // Check against the number of human player slots defined at game creation
                     if (numHumanPlayersInGame >= gameData.numHumanPlayers) { 
                         if (!gameData.players[currentUserId]) { // If current user is not already in, and human slots full
                     ...
                     if (!gameData.players[currentUserId]) { 
                         // Find the next available order index for a human player
                         // This assumes AI players' orders are set and humans fill remaining order slots.
                         // A more robust way would be to just take the current length of playerOrder.
         ...
         function subscribeToGameState(gameId) {
             ...
             unsubscribeGameState = onSnapshot(gameDocRef, async (docSnap) => {
                 if (docSnap.exists()) {
                     ...
                     if (gameData.lastDiceRoll && gameData.players) {
                         Object.values(gameData.players).forEach(player => {
                             if (player.id !== currentUserId && localGameData.players && localGameData.players[player.id]) {
                                 const oldPlayerState = localGameData.players[player.id];
                                 if (oldPlayerState.position !== player.position && 
                                     player.id === gameData.playerOrder[gameData.currentPlayerIndex] && /* Only animate if it was their turn */ 
                                     !player.inDetention && !oldPlayerState.inDetention /* Avoid animation if sent to jail */) {
                     ...
                     if (
                         ...
                         if (currentPlayer && currentPlayer.isAI && !currentPlayer.isBankrupt) {
                             if (!window._aiTurnInProgress) {
                                 ...
                                 setTimeout(async () => {
                                     try {
                                         // Fetch fresh game data before AI acts
         ...
         function resetToSetupScreen() {
             ...
             // if (aiAddTimerId) { // This timer is no longer used
         ...
         function updateGameStatusPanel(gameData) {
             ...
             if (gameData.preGamePhase) {
                  ...
                  if (joinedHumans < gameData.numHumanPlayers) { // Check against expected human players
                     gameStatusMessageP.textContent = `Waiting for human players... (${joinedHumans}/${gameData.numHumanPlayers} joined)`;
                  } else { // All expected humans have joined
                     const allPlayersCount = gameData.playerOrder.length; // Total players including AI
         ...
         function updateControlsBasedOnTurn(gameData) {
             ...
             const amIAI = gameData.players[currentUserId]?.isAI; // AI players shouldn't interact with these client-side controls.
         ...
         function setupBoardFromFirestore(gameData) {
             ...
             // --- Property Swap Logic ---
             if (!window._propertySwapState) window._propertySwapState = { cardA: null, cardB: null, swapTimeout: null, swapActive: false, swapInitiatorPlayerId: null };
             ...
             boardLayout.forEach((s) => {
                 ...
                 // --- Add property swap event listeners ---
                 // Removed single click handler for swap initiation
                 spaceDiv.addEventListener('dblclick', (e) => {
                     handlePropertyCardDoubleClick(s, spaceDiv, gameData);
                 });
             ...
             if (gameData.players && gameData.playerOrder) {
                 Object.values(gameData.players).forEach(player => {
                     if (player && typeof player.id !== 'undefined' && typeof player.order !== 'undefined') { 
                         ...
                         token.style.filter = `drop-shadow(0 0 4px ${playerTokenColor})`; /* Adjusted shadow */
         ...
         function updateBoardDynamicElements(gameData) {
             ...
             // Iterate through all property elements and apply/remove flashing class based on Firestore state
             document.querySelectorAll('.space.property, .space.set-property').forEach(spaceEl => {
                 ...
                 if (gameData.flashingProperties && gameData.flashingProperties.includes(spaceId)) {
                     spaceEl.classList.add('property-flash');
                 } else {
                     spaceEl.classList.remove('property-flash');
                 }
             });
             // End of Property Swap Flashing Update
         }
         ...
         function updateLocalUIFromFirestore(gameData) { 
             ...
             if (gameData.preGamePhase && 
                 ...
                 // Check if all *AI* players have also "rolled" (AI pre-game rolls will be automated by host)
             ...
             if (gameData.preGamePhase && gameData.status === "active") { 
                 updatePreGameRollUI(gameData);
                  // Host automates AI pre-game rolls
         ...
         async function handleRollDiceAction() {
             ...
             if (!localGameData.players || !localGameData.players[currentUserId] || localGameData.players[currentUserId].isAI) { // AI doesn't use this button
                         ...
                         if (landedSpace.type === 'payout' && landedSpace.amount) {
                             ...
                             const currentMoney = updates[`players.${currentUserId}.money`] !== undefined ? updates[`players.${currentUserId}.money`] : playerState.money;
                              if (currentMoney >= landedSpace.amount) {
                                 ...
                                 messages.push(`${playerState.name} fined £${landedSpace.amount} for Crime Spree!`);
                             }
             ...
             setTimeout(async () => { 
                 ...
                 const justWentToJail = localGameData.players[currentUserId]?.inDetention && !currentPlayerStateFromLocal.inDetention; 
         
                 if (!justWentToJail && !rentPaymentRequired) {
         ...
         async function handleEndTurnAction() {
             ...
             // If the action is being called for an AI by the host, currentUserId will be the host,
             // but currentPlayerIdFromState will be the AI.
             // If a human player is ending their turn, currentUserId should match currentPlayerIdFromState.
             ...
             const playerEndingTurnId = currentPlayerIdFromState; // The player whose turn is actually ending.
             const playerState = gameDataForCheck.players[playerEndingTurnId]; 
             if (!playerState) { // Should not happen if above checks are correct
             ...
             if (playerState.isBankrupt) { // Bankrupt players' turns are skipped, this shouldn't be called for them actively.
                 logEvent(`EndTurn: Exiting - Player ${playerEndingTurnId} is bankrupt, turn should have been skipped.`);
                 // This scenario should ideally be handled by the turn progression logic itself.
                 // If we reach here, it's likely a state inconsistency. We'll try to proceed to fix state.
             } else { // Non-bankrupt player
             ...
             try {
                 await runTransaction(db, async (transaction) => {
                     ...
                     if (freshCurrentPlayerId !== playerEndingTurnId) { // Check against the player whose turn we intended to end
                     ...
                     // Re-check conditions for non-bankrupt players in transaction
                     ...
                     while (freshGameData.players[nextPlayerId]?.isBankrupt && attempts < maxAttempts) {
                         ...
                         updates[`players.${nextPlayerId}.playerActionTakenThisTurn`] = false; // Reset for skipped player
                         updates[`players.${nextPlayerId}.doublesRolledInTurn`] = 0; // Reset for skipped player
         ...
         async function handleBuyPropertyAction() {
             ...
             if (localGameData.players[currentUserId]?.isAI) { // AI doesn't use this button
         ...
         async function handlePreGameRollAction(playerIdToRoll = currentUserId) { // Parameter for AI
             if (!currentGameId || !localGameData || !localGameData.preGamePhase) {
              ...
              if (Object.values(localGameData.players).filter(p => !p.isAI).length < localGameData.numHumanPlayers) { // Check against numHumanPlayers
                 if (!playerToRollData.isAI) showMessageModal("Waiting", "Waiting for all human players to join before rolling.");
                 return false;
             }
              if (localGameData.playerOrder.length < localGameData.maxPlayers) { // Check if all players (incl AI) are in playerOrder
             ...
             try {
                 await runTransaction(db, async (transaction) => {
                     ...
                     if (allHaveRolled) { 
                         ...
                         // Host always finalizes, regardless of who rolled last
                         if (freshGameData.hostId === currentUserId || playerToRollData.isAI) { // Host or if AI roll triggers this for host
                              logEvent("All pre-game rolls complete. Host will finalize order.");
                             // Finalization logic is now centralized in finalizePreGameAsHost,
                             // which is called from onSnapshot when conditions are met.
                             // This transaction just records the roll.
                             if (currentUserId === freshGameData.hostId) { //If host is doing this, can trigger finalizePreGameAsHost directly after if all rolled
                                 // This update is now moved to be triggered by onSnapshot after host confirms all rolls
                             }
                         }
                     }
                     transaction.update(gameDocRef, updates);
         ...
         async function automateAIPreGameRolls(gameData) {
             ...
             for (const playerId of gameData.playerOrder) {
                 ...
                 if (player.isAI && (gameData.preGameRolls === undefined || gameData.preGameRolls[playerId] === undefined)) {
                     ...
                     await new Promise(resolve => setTimeout(resolve, 300)); // Small delay between AI rolls
                 }
             }
         }
         ...
         preGameRollButton.onclick = () => handlePreGameRollAction(); // Call with default currentUserId
         ...
         function setupDetentionActionsUI(playerState, gameData) {
             if (!playerState || !playerState.inDetention || !detentionActionsDiv || playerState.isAI) return; // AI doesn't use this UI
             detentionActionsDiv.innerHTML = ''; 
         ...
         function showDevelopPropertyOptions(playerState, gameData) {
             if (!developPropertyContainer || !developPropertyOptionsDiv || !developPropertyNameH3 || playerState.isAI) return; // AI doesn't use
         ...
         async function handleConfirmDevelopment(propertyId, developmentType, cost) { 
             ...
             if (localGameData.players[currentUserId]?.isAI) { // Should not be called by AI directly
         ...
         // --- Updated handleAITurn to make AI roll dice and move ---
         async function handleAITurn(gameData, aiPlayerId) {
                 ...
                 try {
                     if (aiPlayerState.money >= fineAmount) {
                         ...
                         await updateDoc(gameDocRef, {
                             ...
                             bankMoney: (gameData.bankMoney || 0) + fineAmount,
                             lastActionMessage: `${aiPlayerState.name} (AI) paid £${fineAmount} fine and is free.`,
                             updatedAt: serverTimestamp()
                         });
                         logEvent(`AI ${aiPlayerId} paid fine to exit detention.`);
                         await new Promise(resolve => setTimeout(resolve, 500));
                         window._aiTurnInProgress = false;
                         return; // Wait for next turn to roll
                     } else {
                         // Roll for doubles
                         const die1 = Math.floor(Math.random() * 6) + 1;
                         const die2 = Math.floor(Math.random() * 6) + 1;
                         ...
                         const totalRoll = die1 + die2;
                         let updates = {
                             lastDiceRoll: { die1, die2, total: totalRoll, isDoubles },
                             updatedAt: serverTimestamp()
                         };
                         ...
                         if (isDoubles) {
                             ...
                             updates.lastActionMessage = `${aiPlayerState.name} (AI) rolled doubles (${die1},${die2}) and is out of Detention!`;
                         } else {
                             missedTurns++;
                             ...
                             if (missedTurns >= 3) {
                                 ...
                                 updates.lastActionMessage = `${aiPlayerState.name} (AI) failed to roll doubles for 3 turns, couldn't pay fine, and is BANKRUPT!`;
                             } else {
                                 updates.lastActionMessage = `${aiPlayerState.name} (AI) failed to roll doubles (${die1},${die2}). Still in detention. ${3 - missedTurns} attempt(s) left.`;
                             }
                         }
                         await updateDoc(gameDocRef, updates);
                         logEvent(`AI ${aiPlayerId} detention action: ${updates.lastActionMessage}`);
                         await new Promise(resolve => setTimeout(resolve, 500));
                         try {
                             await handleEndTurnAction();
                         } catch (endError) {
                     ...
                     try {
                         ...
                         logEvent(`Error ending turn after detention failure for AI ${aiPlayerId}: ${endError.message}`, endError);
                     }
                     window._aiTurnInProgress = false;
                     return;
                 }
             ...
             // Roll dice and move if not detained
                 ...
                 try {
                     if (audioContextStarted && toneSynth) {
                         try {
                             toneSynth.triggerAttackRelease("C4", "16n", Tone.now());
                             setTimeout(() => { if (toneSynth) toneSynth.triggerAttackRelease("E4", "16n", Tone.now() + 0.1); }, 100);
                         } catch (e) { logEvent(`AI ${aiPlayerId} dice roll sound error: ${e.message}`, e); }
                     }
                     ...
                     try {
                         await runTransaction(db, async (transaction) => {
                             ...
                             if (playerState.inDetention) {
                                 throw new Error("AI in detention.");
                             }
                             ...
                             if (isDoubles && currentDoublesCount === 3) {
                                 ...
                                 if (landedSpace.type === 'payout' && landedSpace.amount) {
                                     const currentMoney = updates[`players.${aiPlayerId}.money`] || playerState.money;
                                     ...
                                     if (currentMoney >= landedSpace.amount) {
                                         ...
                                         updates[`players.${aiPlayerId}.isBankrupt`] = true;
                                         updates.bankMoney = (freshGameData.bankMoney || 0) + currentMoney;
                                         messages.push(`${playerState.name} (AI) couldn't pay £${landedSpace.amount} tax and is BANKRUPT!`);
                                     }
                                 } else if (landedSpace.type === 'crime_spree' && landedSpace.amount) {
                                     const currentMoney = updates[`players.${aiPlayerId}.money`] || playerState.money;
                                     if (currentMoney >= landedSpace.amount) {
                                         ...
                                         updates[`players.${aiPlayerId}.isBankrupt`] = true;
                                         updates.bankMoney = (freshGameData.bankMoney || 0) + currentMoney;
                                         messages.push(`${playerState.name} (AI) couldn't pay £${landedSpace.amount} fine and is BANKRUPT!`);
                                     }
                                 } else if (landedSpace.type === 'go_to_detention') {
                             ...
                             try {
                                 ...
                                 if (gameSnapshot.exists()) {
                                     ...
                                     if (payerState && propertyForRentLayout && propertyForRentData && propertyForRentData.owner && propertyForRentData.owner !== rentPayerId) {
                                         ...
                                         logEvent(`AI ${aiPlayerId} rent payment skipped: Invalid payer, property, or owner data.`);
                                     }
                                 }
                             } catch (rentError) {
                                 logEvent(`AI ${aiPlayerId} rent payment failed: ${rentError.message}`, rentError);
                             }
                         }
                         ...
                         setTimeout(async () => {
                             try {
                                 const freshGameData = (await getDoc(gameDocRef)).data();
                                 if (!freshGameData || !freshGameData.players[aiPlayerId] || freshGameData.players[aiPlayerId].isBankrupt || freshGameData.players[aiPlayerId].inDetention) {
                                     logEvent(`AI ${aiPlayerId} card draw skipped: Player invalid, bankrupt, or in detention.`);
                                     return;
                                 }
         
                                 const finalLandedSpace = freshGameData.boardLayout[landedSpaceId];
                                 if (!finalLandedSpace) {
                                     logEvent(`AI ${aiPlayerId} card draw skipped: Invalid landed space ID ${landedSpaceId}.`);
                                     return;
                                 }
                                 ...
                                 if (!rentPaymentRequired) {
                                     ...
                                     logEvent(`AI ${aiPlayerId} card draw skipped: Rent payment required.`);
                                 }
                             } catch (cardError) {
                         ...
                         // Fallback: reset action flag to allow retry
                         try {
                             await updateDoc(gameDocRef, {
                                 [`players.${aiPlayerId}.playerActionTakenThisTurn`]: false,
                                 lastActionMessage: `${aiPlayerState.name} (AI) roll failed, retrying next turn.`,
                                 updatedAt: serverTimestamp()
                             });
                         } catch (resetError) {
                             logEvent(`AI ${aiPlayerId} action flag reset failed: ${resetError.message}`, resetError);
                         }
                     }
                 } catch (moveError) { // Catch for the outer try block (animatePlayerMove and subsequent logic)
                     logEvent(`AI ${aiPlayerId} move/animation or subsequent action failed: ${moveError.message}`, moveError);
                     // Consider if a general turn end or reset is needed here
                 }
         
                 // End turn only if action taken, bankrupt, in detention, or third doubles
                 setTimeout(async () => {
                     try {
                         ...
                         if (freshGameSnapshot.exists()) {
                             ...
                             if (freshGameData.playerOrder[freshGameData.currentPlayerIndex] === aiPlayerId &&
                                 ...
                                 logEvent(`Host ending turn for AI ${aiPlayerId}. ActionTaken: ${playerState.playerActionTakenThisTurn}, Bankrupt: ${playerState.isBankrupt}, InDetention: ${playerState.inDetention}`);
                                 await handleEndTurnAction();
                             } else if (isDoubles && playerState.doublesRolledInTurn < 3 && !playerState.inDetention && !playerState.isBankrupt) {
                                 logEvent(`AI ${aiPlayerId} turn not ended: Doubles rolled (count: ${playerState.doublesRolledInTurn}), waiting for next roll.`);
                             } else {
                                 logEvent(`AI ${aiPlayerId} turn end skipped: Conditions not met. CurrentPlayer: ${freshGameData.playerOrder[freshGameData.currentPlayerIndex]}, ActionTaken: ${playerState.playerActionTakenThisTurn}, Bankrupt: ${playerState.isBankrupt}, InDetention: ${playerState.inDetention}, Host: ${currentUserId === freshGameData.hostId}`);
                             }
                         } else {
                             logEvent(`AI ${aiPlayerId} turn end skipped: Game document not found.`);
                         }
                     } catch (endError) {
         ...
         // No direct single-click handler for property swap initiation anymore
         function handlePropertyCardClick(space, spaceDiv, gameData) {
             // This function is now mostly for debug or other non-swap related single-click actions
             // The swap initiation is handled by double-click only as per user's request.
             // If you have other single-click behaviors for property cards, they would go here.
         }

         async function handlePropertyCardDoubleClick(space, spaceDiv, gameData) {
            ...
            if (!currentPlayerState || currentPlayerState.isBankrupt || gameData.status !== 'active' || gameData.preGamePhase) {
                ...
                if (!currentPlayerState.isAI) showMessageModal("Swap Error", "Property swap is not allowed in the current game state or if you are bankrupt.");
                return;
            }
            if (!space || (space.type !== 'property' && space.type !== 'set_property')) {
                logEvent("Property swap initiation: Double-clicked space is not a property.");
                if (!currentPlayerState.isAI) showMessageModal("Invalid Card", "You can only swap property cards.");
                return;
            }
        
            const clickedPropData = gameData.propertyData.find(p => p.id === space.id);
            if (!clickedPropData) {
                logEvent("Property swap initiation: Property data not found for clicked space.");
                if (!currentPlayerState.isAI) showMessageModal("Error", "Could not find property data for this card.");
                return;
            }
            ...
            clearPropertySwapFlash(); // Clears local flashing before re-applying based on new Firestore state.
        
            // Step 1: Player double-clicks their own card (instigator's card)
            if (clickedPropData.owner === currentPlayerId) {
                logEvent(`Player ${currentPlayerId} double-clicked their own property: ${space.name}`);
                window._propertySwapState.cardA = { playerId: currentPlayerId, propId: space.id };
                ...
                }, 5000); // 5 seconds to select the second card
        
            }
            // Step 2: Player double-clicks another player's card (target card)
            else if (window._propertySwapState.cardA && window._propertySwapState.swapInitiatorPlayerId === currentPlayerId && clickedPropData.owner !== null && clickedPropData.owner !== currentPlayerId) {
                logEvent(`Player ${currentPlayerId} double-clicked another player's property: ${space.name} (owner: ${clickedPropData.owner})`);
                window._propertySwapState.cardB = { playerId: clickedPropData.owner, propId: space.id };
                ...
                window._propertySwapState.swapTimeout = setTimeout(async () => {
                    ...
                    window._propertySwapState.swapInitiatorPlayerId = null;
                    if (!currentPlayerState.isAI) showMessageModal("Card Swap", "Property swap proposal timed out. Swap cancelled.");
                }, 8000); // 8 seconds for the other player to confirm
                ...
                window._propertySwapState.swapActive = false;
                window._propertySwapState.swapInitiatorPlayerId = null;
                if (!currentPlayerState.isAI) showMessageModal("Card Swap", "You can only propose a swap with an owned property. Swap cancelled.");
                ...
                window._propertySwapState.swapActive = false;
                window._propertySwapState.swapInitiatorPlayerId = null;
                if (!currentPlayerState.isAI) showMessageModal("Card Swap", "Your selected property was deselected. Swap cancelled.");
            } else {
                 // If not in a swap state and not clicking own property
                logEvent("Property swap initiation: Not a valid swap double-click action.");
                if (!currentPlayerState.isAI) showMessageModal("Invalid Action", "Double-click your own property first to start a swap.");
            }
         }

         // This is called by the other player to confirm the swap
         document.addEventListener('click', async function(e) { 
             // Only proceed if a swap is active and a target card (cardB) is defined
             if (!window._propertySwapState.swapActive || !window._propertySwapState.cardB) {
                 return;
             }
         ...
         function clearPropertySwapFlash() {
             // This function now primarily clears local UI elements based on the state that will be
             // synchronized by Firestore. The direct Firestore update is handled in the event listeners.
             // This function will be triggered by onSnapshot updates clearing the Firestore state.
             document.querySelectorAll('.property-flash').forEach(el => el.classList.remove('property-flash'));
             if (window._propertySwapState && window._propertySwapState.swapTimeout) {
                 clearTimeout(window._propertySwapState.swapTimeout);
                 window._propertySwapState.swapTimeout = null;
             }
         }
         async function performPropertySwap(instigator, target) {
             // Firestore transaction: swap ownership of the two properties
             if (!instigator || !target) {
                logEvent("performPropertySwap: Missing instigator or target data.");
                showMessageModal('Swap Error', 'Invalid swap parameters.');
                return;
             }
             if (!currentGameId || !db) {
                logEvent("performPropertySwap: Missing gameId or db connection.");
                showMessageModal('Swap Error', 'Game connection issue.');
                return;
             }
             const gameDocRef = doc(db, 'games', currentGameId);
             try {
                 await runTransaction(db, async (transaction) => {
                     ...
                     const propA = freshGameData.propertyData.find(p => p.id === instigator.propId);
                     const propB = freshGameData.propertyData.find(p => p.id === target.propId);
         
                     if (!propA || !propB) throw new Error('Property not found for swap in current game data.');
                     
                     // Double-check current ownership in transaction to prevent race conditions
                     if (propA.owner !== instigator.playerId || propB.owner !== target.playerId) {
                         throw new Error('Property ownership changed during swap negotiation. Swap cancelled.');
                     }
         
                     // Create a mutable copy of propertyData for updates
                     const updatedPropertyData = freshGameData.propertyData.map(p => ({...p}));
         
                     // Find the actual property objects in the mutable array
                     const updatedPropA = updatedPropertyData.find(p => p.id === instigator.propId);
                     const updatedPropB = updatedPropertyData.find(p => p.id === target.propId);
         
                     // Swap owners
                     updatedPropA.owner = target.playerId;
                     updatedPropB.owner = instigator.playerId;
         
                     // Update players' propertiesOwned arrays in a mutable way for the transaction
                     const updatedPlayers = {...freshGameData.players}; // Create mutable copy of players object
                     
                     if (updatedPlayers[instigator.playerId]) {
                         updatedPlayers[instigator.playerId].propertiesOwned = (updatedPlayers[instigator.playerId].propertiesOwned || []).filter(pid => pid !== instigator.propId);
                         updatedPlayers[instigator.playerId].propertiesOwned.push(target.propId);
                     }
                     if (updatedPlayers[target.playerId]) {
                         updatedPlayers[target.playerId].propertiesOwned = (updatedPlayers[target.playerId].propertiesOwned || []).filter(pid => pid !== target.propId);
                         updatedPlayers[target.playerId].propertiesOwned.push(instigator.propId);
                     }
                     
                     transaction.update(gameDocRef, {
                         propertyData: updatedPropertyData,
                         [`players.${instigator.playerId}.propertiesOwned`]: updatedPlayers[instigator.playerId].propertiesOwned,
                         [`players.${target.playerId}.propertiesOwned`]: updatedPlayers[target.playerId].propertiesOwned,
                         lastActionMessage: `${freshGameData.players[instigator.playerId]?.name || 'Player'} and ${freshGameData.players[target.playerId]?.name || 'Player'} swapped properties!`,
                         flashingProperties: [], // Clear flashing properties after successful swap
                         updatedAt: serverTimestamp(),
                     });
                     logEvent(`Property swap successfully committed in Firestore: ${instigator.propId} (${instigator.playerId}) <-> ${target.propId} (${target.playerId})`);
                     showMessageModal('Swap Complete', 'Properties have been successfully swapped!');
                 });
             } catch (e) {
